...
// Produced by the input-profile-tracker plugin and consumed by the latency-detector plugin.
//
// The tracker selects the observation at the configured percentile of effective input
// (inputTokens * (1 - prefixCacheScore)) and reports both original values so the sidecar
// model receives the same features it was trained on.
type InputProfileInfo struct {
	// inputTokens is the input token count from the representative request.
	inputTokens int
	...
	// percentile ranking but not sent to the sidecar directly.
