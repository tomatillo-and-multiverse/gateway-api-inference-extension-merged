...
func (f *fakePredictor) PredictBulkStrict(_ context.Context, reqs []latencypredictor.PredictionRequest) (*latencypredictor.BulkPredictionResponse, error) {
	...
	if len(f.predictions) != len(reqs) {
		return nil, fmt.Errorf("prediction count mismatch: have %d, want %d", len(f.predictions), len(reqs))
	}
...
func (f *fakePredictor) AddTrainingDataBulk(_ []latencypredictor.TrainingEntry) error { return nil }
...
func TestDetector_Saturation_EmptyPods(t *testing.T) {
	d := &Detector{config: Config{E2ESLOMs: 200}, perEndpointScore: make(map[string]float64), aggregateSaturation: 0.5}
	require.Equal(t, 1.0, d.Saturation(context.Background(), []fwkdl.Endpoint{}))
}

func TestDetector_Saturation_ReturnsCachedValue(t *testing.T) {
	d := &Detector{config: Config{E2ESLOMs: 200}, perEndpointScore: map[string]float64{"ns1/pod1": 0.5}, aggregateSaturation: 0.5}
	got := d.Saturation(context.Background(), []fwkdl.Endpoint{makeDLEndpoint("pod1", 2, 5, 0.3)})
...
func TestDetector_Probe(t *testing.T) {
	...
	// pod1: max(100/200, 20/50) = max(0.5, 0.4) = 0.5
	require.InDelta(t, 0.5, d.perEndpointScore["ns1/pod1"], 1e-6)
	// pod2: max(300/200, 30/50) = max(1.5, 0.6) = 1.5
...
func TestDetector_Probe_Error_KeepsCachedValue(t *testing.T) {
	d := &Detector{
		config:              Config{E2ESLOMs: 200, ProbeInputWords: 512},
		predictor:           &fakePredictor{err: fmt.Errorf("sidecar unavailable")},
		logger:              logr.Discard(),
	...
	d.mu.RLock()
	defer d.mu.RUnlock()
	require.InDelta(t, 0.3, d.aggregateSaturation, 1e-6)
}
...
func TestDetector_Probe_NilMetrics(t *testing.T) {
	...
	d.latestEndpoints = toProbeEndpoints(
		&backendmetrics.FakePodMetrics{
			Metadata: &fwkdl.EndpointMetadata{NamespacedName: k8stypes.NamespacedName{Name: "pod1", Namespace: "ns1"}},
			Metrics:  nil,
		},
	)
	...
	tests := []struct {
		...
		},
		{
			...
			endpoints: func() []framework.Endpoint {
				...
				setLatencyInfo(epBad, -30, 10, 230, 40)
		...
		},
		{
			...
			endpoints: func() []framework.Endpoint {
				...
				setLatencyInfo(epGood, 50, 10, 150, 40)
				epBad := makeSchedulingEndpoint("pod2")
				setLatencyInfo(epBad, 50, -10, 150, 60)
				return []framework.Endpoint{epGood, epBad}
			},
			wantLen: 1,
		},
		{
			...
			endpoints: func() []framework.Endpoint {
				...
				setLatencyInfo(ep, 50, -100, 150, 200)
		...
		},
		{
			...
			config: Config{E2ESLOMs: 200, Headroom: 0.2},
			endpoints: func() []framework.Endpoint {
				ep1 := makeSchedulingEndpoint("pod1")
...
func TestDetector_ProbeLoop_Integration(t *testing.T) {
	...
	predictor := &fakePredictor{
		predictions: []latencypredictor.PredictionResponse{{TTFT: 80, TPOT: 20}},
	}
	...
	require.InDelta(t, 0.4, sat, 1e-6) // 80/200
	cancel()
}

func TestDetector_TypedName(t *testing.T) {
	d := &Detector{typedName: fwkplugin.TypedName{Type: LatencyDetectorType, Name: "my-detector"}}
	require.Equal(t, LatencyDetectorType, d.TypedName().Type)
	require.Equal(t, "my-detector", d.TypedName().Name)
}
...
func TestConfig_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Valid non-streaming", config: Config{E2ESLOMs: 200}},
		{name: "Valid streaming TTFT only", config: Config{TTFTSLOMs: 200}},
		{name: "Valid streaming TTFT + TPOT", config: Config{TTFTSLOMs: 200, TPOTSLOMs: 50}},
		{name: "Invalid - both E2E and TTFT set", config: Config{E2ESLOMs: 200, TTFTSLOMs: 200}, wantErr: true},
		{name: "Invalid - neither set", config: Config{}, wantErr: true},
		{name: "Invalid - TPOT without TTFT", config: Config{E2ESLOMs: 200, TPOTSLOMs: 50}, wantErr: true},
	}
